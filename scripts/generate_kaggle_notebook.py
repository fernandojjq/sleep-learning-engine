...
cells = [
    ...
    code(
        ...
        "",
        "# Seed the ambient library from the package first. The 97",
        "# royalty-free tracks shipped inside the installed wheel are",
        "# copied to /kaggle/working/assets/ambient/ so the render has",
        "# music even when the user has not added a custom Kaggle",
        "# dataset. The user dataset overlay (below) augments this with",
        "# any extra tracks the user uploaded to Kaggle.",
        "_bundled = _pkg_files('sleep_learning_engine').joinpath('assets', 'ambient')",
        "if _bundled.is_dir():",
        "    _n = 0",
        "    for _e in _bundled.iterdir():",
        "        if _e.name.startswith('.') or not _e.name.endswith('.mp3'):",
        "            continue",
        "        _dst = f'{ASSETS}/ambient/{_e.name}'",
        "        if not os.path.exists(_dst):",
        "            with _e.open('rb') as _s, open(_dst, 'wb') as _d:",
        "                shutil.copyfileobj(_s, _d)",
        "            _n += 1",
        "    print(f'Ambient: seeded {_n} tracks from package.')",
        "",
        "# Sanity check: if _n is 0 the wheel is stale. Warn loudly so the",
        "# user does not spend an hour debugging a silent-render issue.",
        "if _n == 0:",
        '    print("  WARNING: no tracks were copied from the package. The render will be silent.")',
        '    print("  Force-reinstall in cell 1 did not pick up the new wheel (1.0.2+).")',
        '    print("  Try restarting the notebook and re-running cell 1.")',
        "",
        "# Overlay the user's Kaggle dataset on top of the bundled one.",
        "# Same filename -> user wins; new files -> added.",
        ...
        '    print(f"User dataset: {copied} tracks overlaid.")',
        '    print(f"Ambient library: {total} tracks total at {AMBIENT_DST}")',
        "else:",
        '    print(f"INFO: {AMBIENT_SRC} not found. Using bundled ambient only.")',
        '    print("  To extend the library, upload an ambient dataset via the + Add data panel.")',
    ),
    code(
    ...
    code(
        ...
        "import glob, os",
        "",
        "candidates = sorted(glob.glob(f'/kaggle/working/output/{OUT_STEM}*.mp4'), key=os.path.getmtime, reverse=True)",
        "if not candidates:",
        ...
        '    marker = " <- newest, download this one" if c == output else ""',
