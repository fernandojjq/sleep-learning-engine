...
cells = [
    ...
    code(
        ...
        "",
        "# Seed the ambient library from the package. The 97 royalty-free",
        "# tracks shipped inside the installed wheel are copied to",
        "# /content/assets/ambient/ on first run. This way the public",
        "# Colab notebook always has music without requiring the user to",
        "# upload anything. (The drive notebook has the same fallback",
        "# before its Drive overlay step.)",
        ...
        "            continue",
        "        _dst = f'/content/assets/ambient/{_e.name}'",
        "        if not os.path.exists(_dst):",
        "            with _e.open('rb') as _s, open(_dst, 'wb') as _d:",
        "                shutil.copyfileobj(_s, _d)",
        "            _n += 1",
        "    print(f'Ambient: seeded {_n} tracks from package (97 total in wheel).')",
        "else:",
        "    print('Ambient: bundled package dir not found, continuing without music.')",
        "",
        ...
        '    print("  Try: Runtime -> Disconnect and delete runtime, then re-run cell 1.")',
        "",
