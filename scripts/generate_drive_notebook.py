...
cells = [
    md(
        ...
        "render workflow: drop the 96 normalised mp3 files into Drive",
        "once, mount Drive on every Colab session, and the rest of the",
        "work is just uploading the new script + image and clicking",
        "**Run all**.",
        "",
        ...
        "My Drive/",
        "└── sleep-learning-engine/",
        "    ├── ambient/      <- 96+ normalised mp3 tracks",
        "    ├── scripts/      <- one or more .txt scripts",
        "    └── images/       <- background images (jpg/png) or short videos (mp4/mov)",
        "```",
        "",
    ...
    code(
        "# 3. Choose your script and background. Three modes - uncomment the one you want.",
        "#",
        "# Mode A: pull from Drive (recommended for recurring renders - just edit the file in Drive)",
        'SCRIPT = f"{DRIVE_ROOT}/scripts/prueba.txt"',
        'BG_IMAGE = f"{DRIVE_ROOT}/images/logo_sleeping_dev.jpeg"',
        "#",
        "# Mode B: upload fresh each session (uncomment to use)",
        "# from google.colab import files",
        "# uploaded = files.upload()  # pick the .txt and image from your machine",
        "# import shutil",
        ...
        "#         fh.write(data)",
        "# import glob",
        "# SCRIPT = glob.glob(f\"{ASSETS}/*.txt\")[0]",
        "# BG_IMAGE = glob.glob(f\"{ASSETS}/*.jpg\") + glob.glob(f\"{ASSETS}/*.jpeg\") + glob.glob(f\"{ASSETS}/*.png\")",
        "# BG_IMAGE = BG_IMAGE[0] if BG_IMAGE else \"\"",
        "",
        "import os",
        'print(f"Script:      {SCRIPT} (exists={os.path.exists(SCRIPT)})")',
        'print(f"Background:  {BG_IMAGE} (exists={os.path.exists(BG_IMAGE)})")',
        "if not os.path.exists(SCRIPT):",
        '    raise SystemExit(f"Script not found: {SCRIPT}. Update SCRIPT path or upload a .txt.")',
    ),
    code(
